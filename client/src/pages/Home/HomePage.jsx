...
const HomePage = () => {
  ...
  return (
    <div className="flex justify-between gap-0 lg:gap-1  h-full">
      {/* Left Side Contents */}
      ...
      <div className="w-full pb-16 sm:pb-0 md:w-2/3 lg:w-2/4 overflow-y-auto custom-scrollbar no-scroll-mobile">
        <div className="flex flex-col items-center px-2 lg:px-0 py-3">
          {/* What's on your mind section */}
          <div className="w-full lg:w-4/5 bg-slate-50 shadow-lg dark:bg-slate-900 rounded-lg p-2">
          ...
          {/* Posts display */}
          {posts.map((post) => (
            <PostCard post={post} key={post.username} />
          ))}
        </div>
      </div>
      
      {/* Right Side Contents */}
      <div className="hidden md:flex justify-end w-1/3 lg:w-1/4">
        <RightContent />
      </div>
    </div>
  );
